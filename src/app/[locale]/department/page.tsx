...
const DepartmentPage = () => {
  ...
  const handleDelete = async (record: GetDepartment) => {
    try {
      ...
      show({
        result: data.result,
        messageDone: 'Xóa phòng ban thành công',
        messageError: 'Xóa phòng ban thất bại',
      });
      GetAllDepartment();
    } catch (error) {
      show({
        result: 1,
        messageError: 'Lỗi xóa phòng ban',
      });
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (editingDepartment) {
        result = await DepartmentAPI.updateDepartment(editingDepartment);
        show({
          result: result.result,
          messageDone: 'Cập nhật phòng ban thành công',
          messageError: 'Cập nhật phòng ban thất bại',
        });
      } else {
        result = await DepartmentAPI.createDepartment(values);
        show({
          result: result.result,
          messageDone: 'Thêm phòng ban thành công',
          messageError: 'Thêm phòng ban thất bại',
        });
      }
  ...
  return (
    <Card className="p-6">
      ...
      {/* Modal Form */}
      <Modal
        title={editingDepartment ? 'Cập nhập phòng ban' : 'Thêm phòng ban'}
        open={modalVisible}
        onOk={handleSave}
        onCancel={closeModal}
        width="60%"
